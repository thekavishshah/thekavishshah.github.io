  ...
  const {
    ...
    onError: (error) => {
      ...
      if (error.message?.includes('quota') || error.message?.includes('exceeded') || error.message?.includes('429')) {
        ...
        toast.error('⚠️ API Quota Exhausted! Free Gemini API limit reached. Please contact Kavish directly or use preset questions. Thank you for understanding! 🙏', {
        ...
        try {
          append({
            role: 'assistant',
            content: '⚠️ **API Quota Exhausted**\n\nFree Gemini API limit reached. Please contact Kavish directly or use preset questions below.',
          });
        } catch (appendError) {
